class Noise:
    ...
    @classmethod
    def perlin_noise(cls, x: float, y: float, scale: float = 1.0) -> float:
        ...
        x1 = Noise.lerp(Noise.grad(aa, xf, yf), Noise.grad(ba, xf - 1, yf), u)
        x2 = Noise.lerp(Noise.grad(ab, xf, yf -1), Noise.grad(bb, xf - 1, yf - 1), u)

        return (Noise.lerp(x1, x2, v) + 1) / 2
